...
object ShopCategoryService {
	...
	def getAll()(implicit conn: Connection): Seq[ShopCategory] = {
		//language=SQL
		val sql = "SELECT * FROM ShopCategory LEFT JOIN ShopCategory p ON ShopCategory.parentCategory_id = p.id JOIN JakonObject jo ON jo.id = ShopCategory.id ORDER BY p.id;"
