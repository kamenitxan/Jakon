...
import io.javalin.config.{Key, MultipartConfig}
import io.javalin.http.{Context, HandlerType, HttpStatus}
import io.javalin.json.JsonMapper
import io.javalin.plugin.ContextPlugin
import io.javalin.router.{Endpoint, Endpoints}
...
class ScalalinServletContext extends Context {
	...
	override def endpoints(): Endpoints = ???

	override def endpoint(): Endpoint = ???

	override def multipartConfig(): MultipartConfig = ???
}
