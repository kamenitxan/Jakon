...
object DBHelper {
	...
	private def selectForeignEntity[T <: JakonObject](fki: ForeignKeyInfo, res: QueryResult[T])(implicit conn: Connection, cls: Class[T]): Unit = {
		...
		val sql = s"SELECT * FROM $className c JOIN JakonObject ON c.id = JakonObject.id WHERE " + "c.id = ? OR ".repeat(fki.ids.size - 1) + "c.id = ?"
			...
			if (r.foreignIds.nonEmpty) {
				r.foreignIds.foreach(fki => {
					...
					if (fki._2.ids.size == 1 && r.entity.id == fki._2.ids.head) {
						...
						val sql = s"SELECT * FROM $className c JOIN JakonObject ON c.id = JakonObject.id WHERE " + "c.id = ? OR ".repeat(fki._2.ids.size - 1) + "c.id = ?"
						val stmt = conn.prepareStatement(sql)
